-- ============================================================================
-- 018_recruiter_trial_manual_review.sql
--
-- Recruiter free-trial requests must always be decided by a platform admin.
--
-- 013 let fn_request_recruiter_trial approve a request itself whenever the
-- computed risk score was low enough ('auto-validator' /
-- 'company_validation_passed'). That silently granted 10 AI Interviews + 25
-- VERIS Screenings without the request ever reaching the review queue in
-- admin.verisnova.com, so nobody could approve, reject, or even see it.
--
-- After this migration every RECRUITER_TRIAL request is created as
-- PENDING_REVIEW and only public.fn_approve_trial_request (called from the
-- admin console) can issue credits. Candidate practice requests are
-- deliberately untouched.
--
-- Rollback: 018_recruiter_trial_manual_review_rollback.sql
-- ============================================================================
...
begin
  ...
  -- Recruiter trials are never auto-approved any more. Every request waits for
  -- a human decision in the admin console (admin.verisnova.com -> Trial
  -- Requests). The risk score is still computed and stored above so reviewers
  -- can triage the queue, but it can no longer hand out credits on its own.
