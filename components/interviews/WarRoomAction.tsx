...
export default function WarRoomAction({ onOpen, candidateName = "candidate", compact = false }: WarRoomActionProps) {
  return (
    <button
      type="button"
      onClick={onOpen}
      className={`hv-preserve-dark group/war relative inline-flex w-full min-w-0 transform-gpu items-center justify-between gap-3 overflow-hidden rounded-2xl border border-cyan-300/14 bg-[linear-gradient(135deg,rgba(4,28,45,0.65),rgba(8,13,26,0.88))] text-left text-slate-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] transition-all duration-200 hover:-translate-y-px hover:border-cyan-300/28 hover:bg-cyan-500/10 hover:text-white hover:shadow-[0_18px_42px_rgba(2,6,23,0.3),0_0_28px_rgba(34,211,238,0.07)] focus:outline-none focus:ring-2 focus:ring-cyan-300/25 ${
        compact ? "px-4 py-3" : "px-4 py-3.5"
      }`}
      aria-label={`Open War Room evidence review for ${candidateName}`}
    >
