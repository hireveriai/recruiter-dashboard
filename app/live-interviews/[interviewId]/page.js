  ...
  return (
    <div className="hv-page-enter min-h-screen bg-slate-950 text-white">
      ...
      <main className="mx-auto max-w-[1200px] space-y-6 px-4 py-8 sm:px-6">
        ...
        {!report && !error ? <p className="text-slate-400">Loading…</p> : null}
        {report ? (
          <>
            <section className={CARD}>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">VERIS Live Interview Report</p>
              ...
              <ul className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
                {report.participants.map((p) => (
                  <li key={p.participantId} className="rounded-xl border border-slate-800 px-3 py-2">
                    <span className="font-medium">{p.displayName}</span>{" "}
                    <span className="text-slate-400">{p.role === "CANDIDATE" ? "Candidate" : label(p.panelRole)}</span>
            ...
            <section className={CARD}>
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300">Interviewer evaluation</p>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {report.scorecards.map((card) => (
                  <div key={card.participantId} className="rounded-xl border border-slate-800 p-4">
                    <p className="font-medium">
                      {card.interviewer} <span className="text-xs text-slate-400">{label(card.panelRole)}</span>
                    </p>
                    {!card.submittedAt ? (
                      <p className="mt-2 text-sm text-slate-400">Scorecard not submitted.</p>
                    ) : (
                        ...
                        {[...card.ratings]
                          ...
                          .map((r, i) => (
                            <li key={i}>
                              ...
                              {r.evidence ? <p className="text-xs text-slate-400">{r.evidence}</p> : null}
                            </li>
                          ))}
                      </ul>
                    )}
            ...
            <section className={CARD}>
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300">VERIS evidence</p>
              <h2 className="mt-1 text-lg font-semibold">What was observed in the interview</h2>
              <p className="mt-1 text-xs text-slate-400">Factual record from the session. VERIS does not score the candidate in a Live Interview.</p>
              <h3 className="mt-4 text-sm font-semibold">Questionnaire coverage</h3>
              <ol className="mt-3 space-y-1 text-sm">
                {report.questions.map((q) => (
                  <li key={q.questionId}>
                    <span className="text-slate-400">{q.order}.</span> {q.text}{" "}
                ...
                <>
                  ...
                  <ul className="mt-1 space-y-1 text-sm text-slate-300">
                    {report.manualQuestions.map((m, i) => (
                      <li key={i}>
                        {m.text} <span className="text-xs text-slate-400">{m.askedBy}</span>
                      </li>
                    ))}
                  </ul>
                </>
              ) : null}
              <p className="mt-3 text-xs text-slate-400">Copilot suggestions requested: {report.copilotSuggestions}</p>
            </section>

            <section className={CARD}>
              <h2 className="text-lg font-semibold">Transcript <span className="text-xs font-normal text-slate-400">VERIS evidence</span></h2>
              {report.transcript.length === 0 ? (
                ...
                <ul className="mt-3 max-h-[480px] space-y-2 overflow-y-auto text-sm">
                  {report.transcript.map((s, i) => (
                    <li key={i}>
                      <span className="mr-2 font-mono text-xs text-slate-400">{clock(s.startMs)}</span>
                      <span className={s.role === "CANDIDATE" ? "font-semibold text-cyan-300" : "font-semibold"}>{s.speaker}:</span> {s.text}
                    </li>
                  ))}
                </ul>
              )}
            ...
            <section className={CARD}>
              <h2 className="text-lg font-semibold">Recordings <span className="text-xs font-normal text-slate-400">VERIS evidence</span></h2>
              {report.recordings.length === 0 ? <p className="mt-2 text-sm text-slate-400">Not recorded.</p> : null}
                  ...
                  <li key={r.recordingId}>
                    ...
                    {r.status === "COMPLETE" ? (
                      playback[r.recordingId] && playback[r.recordingId] !== "error" ? (
                        ...
                        <button type="button" onClick={() => play(r.recordingId)} className="ml-3 rounded-lg border border-slate-700 px-2 py-0.5 text-xs">
            ...
            <section className={CARD}>
              <h2 className="text-lg font-semibold">Timeline <span className="text-xs font-normal text-slate-400">VERIS evidence</span></h2>
